...
- If the two skills suggest conflicting directions, prioritize matching any reference image or `brand_assets/` content first; use the skills to fill gaps, not to override explicit brand material.
...
- If `brand_assets/photos/` contains a `projects.md` (or similarly named file), read it and use those exact project names and descriptions as captions — do not invent generic captions when real ones exist.
